...
# 原图最大扰动范围，文中实验设置为12.0，但在ImageNet数据集和ResNet模型上实验结果噪声过于明显。
...
#   CA2.py中使用0.1，是因为本地模型为inception，样本归一化到(-1,1)
#   文中中使用0.05，是因为在(0,1)语境下表述
#   此处采用0.05，那么在D2A策略实现代码中，需要根据输入样本的bounds将样本归一化到(0,1)后再进行计算
sample_num = 4
